...
// Call the renderForm function to render the plant form
renderPlantForm();
...
const showSowedVegetablesCheckbox = document.querySelector(
  '#show-sowed-vegetables'
);
// Get the associated label element
const labelForShowSowedVegetables = document.querySelector(
  'label[for="show-sowed-vegetables"]'
);

//EventListener for the Add Vegetable Button
addButtonPlant.addEventListener('click', async function (event) {
  ...
  const selectedName = selectedNameOption
    ...
    : '';
  const isSowed = selectedNameOption
    ? selectedNameOption.dataset.sowed === 'true'
    : false;

  try {
    const areaId = document.querySelector('#garden_area_plant').value;
    const baseUrl = 'http://127.0.0.1:8000/api/v1/vegetable_manager/';
    if (isSowed) {
      ...
      if (updatedSowedQuantity >= 0) {
        ...
        console.log(newPlantedVegetable);

        // Create the new planted vegetable and update the sowed vegetable's quantity
        const plantedVegetableUrl =
          'http://127.0.0.1:8000/api/v1/vegetable_manager/create';
      ...
      const newVegetableResponse = await fetch(`${newVegetableUrl}create/`, {
        method: 'POST',
        headers: {
...
// addEventListener for the checkbox to select from sowed vegetables
showSowedVegetablesCheckbox.addEventListener('change', function () {
  if (showSowedVegetablesCheckbox.checked) {
    ...
    labelForShowSowedVegetables.textContent =
      'Uncheck to plant a new Vegetable';
  } else {
  ...
  if (showSowedVegetablesCheckbox.checked) {
    ...
    fetch(baseUrl, requestOptions)
      ...
      .then((data) => {
        ...
        const sowedVegetables = data.filter(
          (vegetable) =>
            vegetable.area.area_id === sowGardenAreaId &&
    ...
    fetch('http://127.0.0.1:8000/api/v1/vegetable_info/', requestOptions)
      .then((response) => response.json())
      .then((data) => {
...
function showSuccessMessage(data, isSowed, isNewVegetable) {
  ...
  if (data && !data.error) {
    ...
    if (isSowed) {
      message.textContent =
        'Error planting the vegetable from a sowed vegetable.';
    } else if (isNewVegetable) {
...
async function fetchVegetableNames() {
  const requestOptions = {
    method: 'GET',
    headers: {
      Authorization: `Bearer ${localStorage.getItem('access_token')}`,
  ...
  const apiUrl = 'http://127.0.0.1:8000/api/v1/vegetable_info/';
  ...
  fetch(apiUrl, requestOptions)
    ...
    .then((data) => {
      ...
      data.forEach((vegetable) => {
        const option = document.createElement('option');
        option.value = vegetable.vegetable_info_id; // Set the value to the vegetable ID
...
function renderPlantForm() {
  ...
  form.innerHTML = `
    <div class="form_line1">
      ...
      <button id="add-vegetable-button-plant" type="submit">Planter</button>
...
async function fetchGardenAreas() {
  const apiUrl = 'http://127.0.0.1:8000/api/v1/area/';
  const sowGardenAreaId = '3e01d962-43c3-42fe-8dd8-f1cca6f4977f';
  try {
